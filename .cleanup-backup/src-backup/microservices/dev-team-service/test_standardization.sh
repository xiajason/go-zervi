...
# 测试配置
SERVICE_NAME="dev-team-service"
SERVICE_PORT="8088"
HEALTH_CHECK_PATH="/health"
VERSION_CHECK_PATH="/version"
INFO_CHECK_PATH="/info"

log_info "开始测试 Dev Team Service 标准化版本..."
...
# 检查角色管理
if grep -q "roles" main_standardized.go; then
    log_success "保持角色管理功能"
else
    log_error "缺少角色管理功能"
fi

# 检查成员身份检查
if grep -q "check-membership" main_standardized.go; then
    log_success "保持成员身份检查功能"
else
    log_error "缺少成员身份检查功能"
fi
...
# 检查开发者功能
if grep -q "dev.GET" main_standardized.go; then
    log_success "保持开发者功能"
else
    log_error "缺少开发者功能"
fi
...
# 7. 总结
log_info "7. 测试总结..."
log_success "Dev Team Service 标准化测试完成"
