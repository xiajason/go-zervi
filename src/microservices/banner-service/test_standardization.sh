...
log_info "开始测试 Banner Service 标准化版本..."
...
# 检查横幅管理
if grep -q "banners.POST" main_standardized.go; then
    log_success "保持横幅管理功能"
else
    log_error "缺少横幅管理功能"
fi

# 检查Markdown内容管理
if grep -q "markdown.POST" main_standardized.go; then
    log_success "保持Markdown内容管理功能"
else
    log_error "缺少Markdown内容管理功能"
fi

# 检查评论系统
if grep -q "comments.POST" main_standardized.go; then
    log_success "保持评论系统功能"
else
    log_error "缺少评论系统功能"
fi
...
# 7. 总结
log_info "7. 测试总结..."
log_success "Banner Service 标准化测试完成"
