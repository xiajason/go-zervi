...
func (tm *Manager) AddMember(req AddMemberRequest) (*AddMemberResponse, error) {
	...
	if err := tm.upsertUserProfile(user.ID, fullName, req.Phone); err != nil {
		fmt.Printf("WARN: 创建团队用户档案失败: %v\n", err)
	}
...
func (tm *Manager) upsertUserProfile(userID uint, fullName, phone string) error {
	nameValue := stringPtr(fullName)
	phoneValue := stringPtr(phone)
	dialect := strings.ToLower(tm.db.Dialector.Name())
	...
	if strings.Contains(dialect, "postgres") {
		return tm.db.Exec(`
			...
			VALUES (?, ?, ?, NOW(), NOW())
			ON CONFLICT (user_id)
			DO UPDATE SET real_name = EXCLUDED.real_name, phone = EXCLUDED.phone, updated_at = NOW()
		`, userID, nameValue, phoneValue).Error
	}
