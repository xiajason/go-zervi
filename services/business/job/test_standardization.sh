...
log_info "开始测试 Job Service 标准化版本..."
...
# 检查职位管理
if grep -q "jobs.POST" main_standardized.go; then
    log_success "保持职位管理功能"
else
    log_error "缺少职位管理功能"
fi

# 检查职位申请
if grep -q "apply" main_standardized.go; then
    log_success "保持职位申请功能"
else
    log_error "缺少职位申请功能"
fi

# 检查公司职位管理
if grep -q "companies" main_standardized.go; then
    log_success "保持公司职位管理功能"
else
    log_error "缺少公司职位管理功能"
fi
...
# 检查管理员功能
if grep -q "admin.GET" main_standardized.go; then
    log_success "保持管理员功能"
else
    log_error "缺少管理员功能"
fi
...
# 检查服务间集成
if grep -q "CompanyClient" main_standardized.go; then
...
# 7. 总结
log_info "7. 测试总结..."
log_success "Job Service 标准化测试完成"
