...
log_info "开始测试 Company Service 标准化版本..."
...
# 检查基础企业管理
if grep -q "companies.POST" main_standardized.go; then
    log_success "保持基础企业管理功能"
else
    log_error "缺少基础企业管理功能"
fi
...
# 检查文档管理
if grep -q "NewDocumentAPI" main_standardized.go; then
    log_success "保持文档管理功能"
else
    log_error "缺少文档管理功能"
fi

# 检查企业画像
if grep -q "NewCompanyProfileAPI" main_standardized.go; then
    log_success "保持企业画像功能"
else
    log_error "缺少企业画像功能"
fi

# 检查AI配额管理
if grep -q "NewQuotaAPI" main_standardized.go; then
    log_success "保持AI配额管理功能"
else
    log_error "缺少AI配额管理功能"
fi

# 检查权限管理
if grep -q "NewCompanyPermissionManager" main_standardized.go; then
    log_success "保持权限管理功能"
else
    log_error "缺少权限管理功能"
fi

# 检查数据同步
if grep -q "NewCompanyDataSyncService" main_standardized.go; then
    log_success "保持数据同步功能"
else
    log_error "缺少数据同步功能"
fi

# 检查增强功能
if grep -q "setupCompanyEnhancedRoutes" main_standardized.go; then
    log_success "保持增强功能"
else
    log_error "缺少增强功能"
fi
...
# 7. 总结
log_info "7. 测试总结..."
log_success "Company Service 标准化测试完成"
