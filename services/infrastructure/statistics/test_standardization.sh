...
log_info "开始测试 Statistics Service 标准化版本..."
...
# 检查基础统计分析
if grep -q "overview" main_standardized.go; then
    log_success "保持基础统计分析功能"
else
    log_error "缺少基础统计分析功能"
fi
...
# 检查用户个人统计
if grep -q "user/:id" main_standardized.go; then
    log_success "保持用户个人统计功能"
else
    log_error "缺少用户个人统计功能"
fi

# 检查管理员统计面板
if grep -q "admin" main_standardized.go; then
    log_success "保持管理员统计面板功能"
else
    log_error "缺少管理员统计面板功能"
fi

# 检查增强功能
if grep -q "setupStatisticsEnhancedRoutes" main_standardized.go; then
    log_success "保持增强功能"
else
    log_error "缺少增强功能"
fi
...
# 7. 总结
log_info "7. 测试总结..."
log_success "Statistics Service 标准化测试完成"
