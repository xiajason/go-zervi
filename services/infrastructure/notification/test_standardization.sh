...
log_info "开始测试 Notification Service 标准化版本..."
...
# 检查通知管理
if grep -q "notifications.GET" main_standardized.go; then
    log_success "保持通知管理功能"
else
    log_error "缺少通知管理功能"
fi

# 检查通知业务逻辑
if grep -q "NotificationBusiness" main_standardized.go; then
    log_success "保持通知业务逻辑功能"
else
    log_error "缺少通知业务逻辑功能"
fi

# 检查服务间集成
if grep -q "ServiceIntegration" main_standardized.go; then
    log_success "保持服务间集成功能"
else
    log_error "缺少服务间集成功能"
fi

# 检查成本控制通知
if grep -q "CostControlNotification" main_standardized.go; then
    log_success "保持成本控制通知功能"
else
    log_error "缺少成本控制通知功能"
fi

# 检查集成API
if grep -q "IntegrationAPI" main_standardized.go; then
    log_success "保持集成API功能"
else
    log_error "缺少集成API功能"
fi
...
# 检查权限控制
if grep -q "User ID not found" main_standardized.go; then
...
# 7. 总结
log_info "7. 测试总结..."
log_success "Notification Service 标准化测试完成"
